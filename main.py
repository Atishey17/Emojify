...
import numpy as ns
#faceMesh is used to provide functionality in detecting facial landmarks
cap = cv2.VideoCapture(0)
#VideoCapture is used to get input from our camera
def draw_Arc(img, x, y, rotation, radius):
    ...
    cv2.ellipse(img, center,axis,angle,startAngle,endAngle,(255,255,255),int(radius/2))

detector = FaceMesh.FaceMeshDetector()

while True:
    _, img = cap.read()
#Reads a frame from the video capture and assigns it to the img variable.
    img, faces = detector.findFaceMesh(img, draw=False)
# True for showing the outline of face and false for not showing.
    if faces:
        for j in range(len(faces)):
            for i in range(len(faces[j])):
                faceLength, face_pos = findDistance(faces[j][face['faceUp']], faces[j][face['faceDown']], img)
                cv2.ellipse(img, (face_pos[0], face_pos[1]-30), (int(faceLength/1.5), int(faceLength/1.5)+10),
                            find_rotation(faces[j][face['faceUp']], faces[j][face['faceDown']]), 0, 360, (255, 218, 185), -1)

                Right_eye_Length, R_eye_pos = findDistance(faces[j][rightEye['eyeUp']], faces[j][rightEye['eyeDown']], img)
                ...
                Left_eye_Length, L_eye_pos = findDistance(faces[j][leftEye['eyeUp']], faces[j][leftEye['eyeDown']], img)
                ...
                MouthLength1, Mouth_pos = findDistance(faces[j][mouth['mouthRight']], faces[j][mouth['mouthLeft']], img)
                MouthLength2, _ = findDistance(faces[j][mouth['mouthUp']], faces[j][mouth['mouthDown']], img)
                ...
                right_eyebrow_polygon = ns.array([right_eyebrow], ns.int32)
                ...
                left_eyebrow_polygon = ns.array([left_eyebrow], ns.int32)
